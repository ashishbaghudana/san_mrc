...
DigitsMapper = {'0': 'zero', '1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', '7': 'seven',
...
def build_vocab(data, glove_vocab=None, sort_all=False, thread=24, clean_on=False, cl_on=True):
    if cl_on:
        ...
        nlp = spacy.load('en_core_web_md', disable=['vectors', 'textcat', 'tagger', 'ner', 'parser'])
    ...
    if sort_all:
        ...
        vocab += sorted([w for w in doc_counter.keys() - query_counter.keys() if w in glove_vocab], key=counter.get,
                        reverse=True)

...
class SAN(nn.Module):
    ...
    def forward(self, x, h0, x_mask):
        ...
        if self.mem_type == 1:
            ...
            start_scores_list = [mask[idx].view(x.size(0), 1).expand_as(inp) * F.softmax(inp, 1) for idx, inp in
                                 enumerate(start_scores_list)]
            end_scores_list = [mask[idx].view(x.size(0), 1).expand_as(inp) * F.softmax(inp, 1) for idx, inp in
                               enumerate(end_scores_list)]
